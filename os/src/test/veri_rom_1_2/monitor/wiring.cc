...
static class Hdl : public Vmonitor {
	public: Hdl() { Vmonitor::write_lock = 0; }
} hdl;
void evaluate_hdl() { if (!Verilated::gotFinish()) hdl.eval(); }

/**
 * Use verilator_env tools to connect HDL to its enviroment
 */
...
/**
 * Connect emulator service to the tools
 */
